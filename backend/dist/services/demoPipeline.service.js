...
exports.generateDemoArtifactsFromCsv = generateDemoArtifactsFromCsv;
const client_1 = require("../db/client");
const DEMO_REVIEWER = 'demo@autoflow.local';
function scoreWsjf(value) {
    return (value.bv + value.tc + value.rr) / value.effort;
}
function findMatches(rows, regex) {
...
async function generateDemoArtifactsFromCsv(rows) {
    await (0, client_1.query)(`DELETE FROM user_stories WHERE epic_id IN (SELECT id FROM epics WHERE status = 'demo_generated')`);
    await (0, client_1.query)(`DELETE FROM epics WHERE status = 'demo_generated'`);
    await (0, client_1.query)(`DELETE FROM brds WHERE reviewer_email = $1`, [DEMO_REVIEWER]);
    await (0, client_1.query)(`DELETE FROM raw_feedback WHERE source = 'sheet'`);
    for (const row of rows) {
        await (0, client_1.query)(`INSERT INTO raw_feedback (source, external_id, text, author_id, author_tier, sentiment, sentiment_score, is_duplicate, collected_at)
       VALUES ('sheet', $1, $2, $3, $4, 'neutral', 0.5, false, NOW())`, [row.ID || null, row.Feedback || '', row.User || 'anonymous', row.Tier || 'free']);
    }
    ...
    for (let i = 0; i < chosenThemes.length; i++) {
        ...
        await (0, client_1.query)(`INSERT INTO brds (
